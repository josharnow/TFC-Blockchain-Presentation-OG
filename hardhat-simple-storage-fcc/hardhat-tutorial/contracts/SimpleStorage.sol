...
contract SimpleStorage { // A "contract" is analagous to a class in other languages.
    // Some data types:
    bool hasFavoriteNumber = true; // Variable declaration and initialization. State variabls are designated as "internal" unless otherwise sepecified, meaning that they cannot be seen externally once the contract is deployed to the blockchain.
    ...
    People[] public people; // Declares and initializes an empty dynamic array.

    mapping(string => uint) public nameToFavoriteNumber; // Mappings are data structures where a key is mapped to a single value. This is similar to a dictionary object.
    ...
    function addPerson(string memory _name, uint _favoriteNumber) public { // "memory" indicates that the variable only exists temporarily, while the transaction is called. Since the "_name" variable isn't needed after the function is run, we cast it as a "memory" type. Variables other than arrays, structs, and mapping types are implicitly cast as a "memory" type.
    // If unspecified, arrays, structs, and mapping types are cast as "storage" types. These exist outside of the function and can be modified.
    // We could also specify the _name variable as a "calldata" type. This is similar to "memory," but makes the variable immutable.
        people.push(People(_favoriteNumber, _name)); // Adds a new People object with the specified attributes to the people array
